...
Define_Module(Fabrication);


const int KIND_DELAYEDSEND = 100;

void Fabrication::initialize(int stage)
{
...
void Fabrication::handleMessageWhenUp(cMessage *msg)
{
...
void Fabrication::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void Fabrication::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result Fabrication::ensureRouteForDatagram(Packet *datagram)
{
...
bool Fabrication::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void Fabrication::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address Fabrication::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void Fabrication::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void Fabrication::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void Fabrication::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> Fabrication::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> Fabrication::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (true) {    //rreq->getDestAddr() == getSelfIPAddress()
        ...
        EV_INFO << "Fabrication Attack" << endl;
        std::cout << "Fabrication Attack from " << getSelfIPAddress().str() << "\n";
...
const Ptr<Rrep> Fabrication::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void Fabrication::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void Fabrication::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void Fabrication::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void Fabrication::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void Fabrication::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void Fabrication::socketClosed(UdpSocket *socket)
{
...
void Fabrication::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *Fabrication::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void Fabrication::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void Fabrication::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> Fabrication::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void Fabrication::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void Fabrication::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void Fabrication::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void Fabrication::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void Fabrication::clearState()
{
...
void Fabrication::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void Fabrication::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void Fabrication::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void Fabrication::completeRouteDiscovery(const L3Address& target)
{
...
void Fabrication::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> Fabrication::createHelloMessage()
{
...
void Fabrication::sendHelloMessagesIfNeeded()
{
...
void Fabrication::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void Fabrication::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void Fabrication::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result Fabrication::datagramForwardHook(Packet *datagram)
{
...
void Fabrication::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void Fabrication::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool Fabrication::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> Fabrication::createRREPACK()
{
...
void Fabrication::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void Fabrication::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void Fabrication::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void Fabrication::handleBlackListTimer()
{
