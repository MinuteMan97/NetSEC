...
void FloodingRSA::initialize(int stage)
{
...
void FloodingRSA::handleMessageWhenUp(cMessage *msg)
{
...
void FloodingRSA::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void FloodingRSA::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result FloodingRSA::ensureRouteForDatagram(Packet *datagram)
{
...
FloodingRSA::FloodingRSA()
{
}

bool FloodingRSA::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void FloodingRSA::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address FloodingRSA::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void FloodingRSA::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void FloodingRSA::sendRREQ(const Ptr<RreqSec>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void FloodingRSA::FloodingAttack()
{
    //creazione ip falso
...
void FloodingRSA::sendRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RreqSec> FloodingRSA::createRREQ(const L3Address& destAddr)
{
...
const Ptr<RrepSec> FloodingRSA::createRREP(const Ptr<RreqSec>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (rreq->getDestAddr() == getSelfIPAddress()) {    // rreq->getDestAddr() == getSelfIPAddress()
...
const Ptr<RrepSec> FloodingRSA::createGratuitousRREP(const Ptr<RreqSec>& rreq, IRoute *originatorRoute)
{
...
void FloodingRSA::handleRREP(const Ptr<RrepSec>& rrep, const L3Address& sourceAddr)
{
...
void FloodingRSA::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void FloodingRSA::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void FloodingRSA::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void FloodingRSA::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void FloodingRSA::socketClosed(UdpSocket *socket)
{
...
void FloodingRSA::handleRREQ(const Ptr<RreqSec>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    rreq->setHopCount(rreq->getHopCount() + 1); //POTENZIALE MODIFICA
...
IRoute *FloodingRSA::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void FloodingRSA::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void FloodingRSA::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> FloodingRSA::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void FloodingRSA::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void FloodingRSA::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void FloodingRSA::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void FloodingRSA::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void FloodingRSA::clearState()
{
...
void FloodingRSA::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void FloodingRSA::forwardRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void FloodingRSA::forwardRREQ(const Ptr<RreqSec>& rreq, unsigned int timeToLive)
{
...
void FloodingRSA::completeRouteDiscovery(const L3Address& target)
{
...
void FloodingRSA::sendGRREP(const Ptr<RrepSec>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RrepSec> FloodingRSA::createHelloMessage()
{
...
void FloodingRSA::sendHelloMessagesIfNeeded()
{
...
void FloodingRSA::handleHelloMessage(const Ptr<RrepSec>& helloMessage)
{
...
void FloodingRSA::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void FloodingRSA::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result FloodingRSA::datagramForwardHook(Packet *datagram)
{
...
void FloodingRSA::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void FloodingRSA::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool FloodingRSA::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> FloodingRSA::createRREPACK()
{
...
void FloodingRSA::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void FloodingRSA::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void FloodingRSA::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void FloodingRSA::handleBlackListTimer()
{
