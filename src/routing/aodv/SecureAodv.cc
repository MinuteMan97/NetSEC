...
std::string sign(std::string message, std::string my_ip) {
    ...
    std::string myIpBase64;
    CryptoPP::StringSource st(my_ip, true,new CryptoPP::Base64Encoder(new CryptoPP::StringSink(myIpBase64)));
...
bool verify(int length, std::string message, std::string signatureBase64, std::string originator_ip) {
    std::string originatorIpBase64;
    CryptoPP::StringSource st(originator_ip, true,new CryptoPP::Base64Encoder(new CryptoPP::StringSink(originatorIpBase64)));

    std::string filename = "/home/veins/workspace.omnetpp/NetSecProject/src/keys/pubkey_"+ originatorIpBase64 +".txt";
...
void SecureAodv::handleMessageWhenUp(cMessage *msg)
{
...
void SecureAodv::processPacket(Packet *packet)
{
    ...
    switch (packetType) {
        ...
        case RREQ_IPv6:
            ...
            handleRREQ(CHK(dynamicPtrCast<RreqSec>(aodvPacket->dupShared())), sourceAddr, arrivalPacketTTL);
            delete packet;
            return;
        ...
        case RREP_IPv6:
            checkIpVersionAndPacketTypeCompatibility(packetType);
            handleRREP(CHK(dynamicPtrCast<RrepSec>(aodvPacket->dupShared())), sourceAddr);
            delete packet;
            return;
...
INetfilter::IHook::Result SecureAodv::ensureRouteForDatagram(Packet *datagram)
{
...
{
    //std::cout << getSelfIPAddress().str();
    //GenKeyPair();
}

bool SecureAodv::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void SecureAodv::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address SecureAodv::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void SecureAodv::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void SecureAodv::sendRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RreqSec> SecureAodv::createRREQ(const L3Address& destAddr)
{
    ...
    std::string my_ip = rreqPacket->getOriginatorAddr().str();
    std::string message = destAddrString;

    std::cout << "my ip is -->" << my_ip << "\n";
    std:cout << "destination ip is -->" << message << "\n";
...
const Ptr<RrepSec> SecureAodv::createRREP(const Ptr<RreqSec>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    auto rrep = makeShared<RrepSec>(); // TODO: "AODV-RREP");
...
const Ptr<RrepSec> SecureAodv::createGratuitousRREP(const Ptr<RreqSec>& rreq, IRoute *originatorRoute)
{
    ASSERT(originatorRoute != nullptr);
    auto grrep = makeShared<RrepSec>(); // TODO: "AODV-GRREP");
...
void SecureAodv::handleRREP(const Ptr<RrepSec>& rrep, const L3Address& sourceAddr)
{
    ...
    bool result = verify(len, messageBase64, signature,rrep->getOriginatorAddr().str());
    ...
    if (true == result) {
        ...
        EV_INFO << "blacklist for node " << rrep->getOriginatorAddr().str().c_str() << endl;
    ...
    if (getSelfIPAddress() != rrep->getOriginatorAddr()) {
        ...
        if (originatorRoute && originatorRoute->getSource() == this) {
            ...
            if (simTime() > rebootTime + deletePeriod || rebootTime == 0.1) {
                ...
                auto outgoingRREP = dynamicPtrCast<RrepSec>(rrep->dupShared());
...
void SecureAodv::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void SecureAodv::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void SecureAodv::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void SecureAodv::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void SecureAodv::socketClosed(UdpSocket *socket)
{
...
void SecureAodv::handleRREQ(const Ptr<RreqSec>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    bool result = verify(len, messageBase64, signature, rreq->getOriginatorAddr().str());
    ...
    if (true == result) {
        ...
        EV_INFO << "blacklist for node " << rreq->getOriginatorAddr().str().c_str() << endl;
        blacklist.insert({rreq->getOriginatorAddr(),simTime()});
        cout << "Message Verification Failed" << endl;
        return;
    }
    ...
    if (timeToLive > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0.1)) {
        if (destRouteData)
            rreq->setDestSeqNum(std::max(destRouteData->getDestSeqNum(), rreq->getDestSeqNum()));
        rreq->setUnknownSeqNumFlag(false);

        auto outgoingRREQ = dynamicPtrCast<RreqSec>(rreq->dupShared());
...
IRoute *SecureAodv::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void SecureAodv::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void SecureAodv::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> SecureAodv::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void SecureAodv::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
    ...
    if (unreachableNeighbors.size() > 0 && (simTime() > rebootTime + deletePeriod || rebootTime == 0.1)) {
...
void SecureAodv::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void SecureAodv::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void SecureAodv::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void SecureAodv::clearState()
{
...
void SecureAodv::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void SecureAodv::forwardRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void SecureAodv::forwardRREQ(const Ptr<RreqSec>& rreq, unsigned int timeToLive)
{
...
void SecureAodv::completeRouteDiscovery(const L3Address& target)
{
...
void SecureAodv::sendGRREP(const Ptr<RrepSec>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RrepSec> SecureAodv::createHelloMessage()
{
    ...
    auto helloMessage = makeShared<RrepSec>(); // TODO: "AODV-HelloMsg");
...
void SecureAodv::sendHelloMessagesIfNeeded()
{
...
void SecureAodv::handleHelloMessage(const Ptr<RrepSec>& helloMessage)
{
...
void SecureAodv::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result SecureAodv::datagramForwardHook(Packet *datagram)
{
...
void SecureAodv::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void SecureAodv::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool SecureAodv::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> SecureAodv::createRREPACK()
{
...
void SecureAodv::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void SecureAodv::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void SecureAodv::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void SecureAodv::handleBlackListTimer()
{
...
SecureAodv::~SecureAodv()
{
    ...
    //rimuoviamo le chiavi create
    std::string message = getSelfIPAddress().str();
    std::string messageBase64;
    CryptoPP::StringSource ss(message, true,new CryptoPP::Base64Encoder(new CryptoPP::StringSink(messageBase64)));
    std::string filename = "/home/veins/workspace.omnetpp/NetSecProject/src/keys/privkey_"+ messageBase64 +".txt";
    std::remove(filename.c_str());
    filename = "/home/veins/workspace.omnetpp/NetSecProject/src/keys/pubkey_"+ messageBase64 +".txt";
    std::remove(filename.c_str());
}
