...
void BlackHoleRSA::initialize(int stage)
{
...
void BlackHoleRSA::handleMessageWhenUp(cMessage *msg)
{
...
void BlackHoleRSA::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void BlackHoleRSA::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result BlackHoleRSA::ensureRouteForDatagram(Packet *datagram)
{
...
BlackHoleRSA::BlackHoleRSA()
{
}

bool BlackHoleRSA::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void BlackHoleRSA::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address BlackHoleRSA::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void BlackHoleRSA::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void BlackHoleRSA::sendRREQ(const Ptr<RreqSec>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void BlackHoleRSA::sendRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RreqSec> BlackHoleRSA::createRREQ(const L3Address& destAddr)
{
...
const Ptr<RrepSec> BlackHoleRSA::createRREP(const Ptr<RreqSec>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
...
const Ptr<RrepSec> BlackHoleRSA::createGratuitousRREP(const Ptr<RreqSec>& rreq, IRoute *originatorRoute)
{
...
void BlackHoleRSA::handleRREP(const Ptr<RrepSec>& rrep, const L3Address& sourceAddr)
{
...
void BlackHoleRSA::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void BlackHoleRSA::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void BlackHoleRSA::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void BlackHoleRSA::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void BlackHoleRSA::socketClosed(UdpSocket *socket)
{
...
void BlackHoleRSA::handleRREQ(const Ptr<RreqSec>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
...
IRoute *BlackHoleRSA::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void BlackHoleRSA::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void BlackHoleRSA::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> BlackHoleRSA::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void BlackHoleRSA::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void BlackHoleRSA::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void BlackHoleRSA::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void BlackHoleRSA::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void BlackHoleRSA::clearState()
{
...
void BlackHoleRSA::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void BlackHoleRSA::forwardRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void BlackHoleRSA::forwardRREQ(const Ptr<RreqSec>& rreq, unsigned int timeToLive)
{
...
void BlackHoleRSA::completeRouteDiscovery(const L3Address& target)
{
...
void BlackHoleRSA::sendGRREP(const Ptr<RrepSec>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RrepSec> BlackHoleRSA::createHelloMessage()
{
...
void BlackHoleRSA::sendHelloMessagesIfNeeded()
{
...
void BlackHoleRSA::handleHelloMessage(const Ptr<RrepSec>& helloMessage)
{
...
void BlackHoleRSA::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void BlackHoleRSA::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result BlackHoleRSA::datagramForwardHook(Packet *datagram)
{
...
void BlackHoleRSA::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void BlackHoleRSA::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool BlackHoleRSA::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> BlackHoleRSA::createRREPACK()
{
...
void BlackHoleRSA::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void BlackHoleRSA::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void BlackHoleRSA::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void BlackHoleRSA::handleBlackListTimer()
{
