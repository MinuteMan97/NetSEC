...
Define_Module(BlackHole);

const int KIND_DELAYEDSEND = 100;

void BlackHole::initialize(int stage)
{
...
void BlackHole::handleMessageWhenUp(cMessage *msg)
{
...
void BlackHole::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void BlackHole::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result BlackHole::ensureRouteForDatagram(Packet *datagram)
{
...
bool BlackHole::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void BlackHole::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address BlackHole::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void BlackHole::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void BlackHole::sendRREQ(const Ptr<Rreq>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void BlackHole::sendRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rreq> BlackHole::createRREQ(const L3Address& destAddr)
{
...
const Ptr<Rrep> BlackHole::createRREP(const Ptr<Rreq>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (true) {    // Mandiamo subito la risposta
        ...
        // sono io il nodo più prossimo a quello che cerchi
        rrep->setHopCount(1);
...
const Ptr<Rrep> BlackHole::createGratuitousRREP(const Ptr<Rreq>& rreq, IRoute *originatorRoute)
{
...
void BlackHole::handleRREP(const Ptr<Rrep>& rrep, const L3Address& sourceAddr)
{
...
void BlackHole::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void BlackHole::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void BlackHole::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void BlackHole::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void BlackHole::socketClosed(UdpSocket *socket)
{
...
void BlackHole::handleRREQ(const Ptr<Rreq>& rreq, const L3Address& sourceAddr, unsigned int timeToLive)
{
    ...
    // Per qualsiasi pacchetto che riceviamo dobbiamo generare una RREP malevola
    if (true) {
...
IRoute *BlackHole::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void BlackHole::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void BlackHole::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> BlackHole::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void BlackHole::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void BlackHole::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void BlackHole::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void BlackHole::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void BlackHole::clearState()
{
...
void BlackHole::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void BlackHole::forwardRREP(const Ptr<Rrep>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void BlackHole::forwardRREQ(const Ptr<Rreq>& rreq, unsigned int timeToLive)
{
...
void BlackHole::completeRouteDiscovery(const L3Address& target)
{
...
void BlackHole::sendGRREP(const Ptr<Rrep>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<Rrep> BlackHole::createHelloMessage()
{
...
void BlackHole::sendHelloMessagesIfNeeded()
{
...
void BlackHole::handleHelloMessage(const Ptr<Rrep>& helloMessage)
{
...
void BlackHole::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result BlackHole::datagramForwardHook(Packet *datagram)
{
...
void BlackHole::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void BlackHole::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool BlackHole::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> BlackHole::createRREPACK()
{
...
void BlackHole::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void BlackHole::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void BlackHole::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void BlackHole::handleBlackListTimer()
{
