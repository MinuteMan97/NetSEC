...
class INET_API FabricationRSA : public RoutingProtocolBase, public NetfilterBase::HookBase, public UdpSocket::ICallback, public cListener
{
  protected:
    /*
  ...
  protected:
    ...
    const Ptr<RrepSec> createHelloMessage();
    const Ptr<RreqSec> createRREQ(const L3Address& destAddr);
    const Ptr<RrepSec> createRREP(const Ptr<RreqSec>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& sourceAddr);
    const Ptr<RrepSec> createGratuitousRREP(const Ptr<RreqSec>& rreq, IRoute *originatorRoute);
    const Ptr<Rerr> createRERR(const std::vector<UnreachableNode>& unreachableNodes);

    /* Control Packet handlers */
    void handleRREP(const Ptr<RrepSec>& rrep, const L3Address& sourceAddr);
    void handleRREQ(const Ptr<RreqSec>& rreq, const L3Address& sourceAddr, unsigned int timeToLive);
    void handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr);
    void handleHelloMessage(const Ptr<RrepSec>& helloMessage);
    ...
    void sendRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive);
    void sendGRREP(const Ptr<RrepSec>& grrep, const L3Address& destAddr, unsigned int timeToLive);

    /* Control Packet forwarders */
    void forwardRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive);
    void forwardRREQ(const Ptr<RreqSec>& rreq, unsigned int timeToLive);
