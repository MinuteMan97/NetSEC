...
void FabricationRSA::initialize(int stage)
{
...
void FabricationRSA::handleMessageWhenUp(cMessage *msg)
{
...
void FabricationRSA::checkIpVersionAndPacketTypeCompatibility(AodvControlPacketType packetType)
{
    switch (packetType) {
        case RREQ:
        case RREP:
...
void FabricationRSA::processPacket(Packet *packet)
{
...
INetfilter::IHook::Result FabricationRSA::ensureRouteForDatagram(Packet *datagram)
{
...
FabricationRSA::FabricationRSA()
{
}

bool FabricationRSA::hasOngoingRouteDiscovery(const L3Address& target)
{
    return waitForRREPTimers.find(target) != waitForRREPTimers.end();
}

void FabricationRSA::startRouteDiscovery(const L3Address& target, unsigned timeToLive)
{
...
L3Address FabricationRSA::getSelfIPAddress() const
{
    return routingTable->getRouterIdAsGeneric();
}

void FabricationRSA::delayDatagram(Packet *datagram)
{
    const auto& networkHeader = getNetworkProtocolHeader(datagram);
...
void FabricationRSA::sendRREQ(const Ptr<RreqSec>& rreq, const L3Address& destAddr, unsigned int timeToLive)
{
...
void FabricationRSA::sendRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RreqSec> FabricationRSA::createRREQ(const L3Address& destAddr)
{
...
const Ptr<RrepSec> FabricationRSA::createRREP(const Ptr<RreqSec>& rreq, IRoute *destRoute, IRoute *originatorRoute, const L3Address& lastHopAddr)
{
    ...
    if (true) {
        // rreq->getDestAddr() == getSelfIPAddress()
...
const Ptr<RrepSec> FabricationRSA::createGratuitousRREP(const Ptr<RreqSec>& rreq, IRoute *originatorRoute)
{
...
void FabricationRSA::handleRREP(const Ptr<RrepSec>& rrep, const L3Address& sourceAddr)
{
...
void FabricationRSA::updateRoutingTable(IRoute *route, const L3Address& nextHop, unsigned int hopCount, bool hasValidDestNum, unsigned int destSeqNum, bool isActive, simtime_t lifeTime)
{
...
void FabricationRSA::sendAODVPacket(const Ptr<AodvControlPacket>& aodvPacket, const L3Address& destAddr, unsigned int timeToLive, double delay)
{
    ASSERT(timeToLive != 0);
...
void FabricationRSA::socketDataArrived(UdpSocket *socket, Packet *packet)
{
    // process incoming packet
    processPacket(packet);
}

void FabricationRSA::socketErrorArrived(UdpSocket *socket, Indication *indication)
{
    EV_WARN << "Ignoring UDP error report " << indication->getName() << endl;
    delete indication;
}

void FabricationRSA::socketClosed(UdpSocket *socket)
{
...
IRoute *FabricationRSA::createRoute(const L3Address& destAddr, const L3Address& nextHop,
...
void FabricationRSA::receiveSignal(cComponent *source, simsignal_t signalID, cObject *obj, cObject *details)
{
...
void FabricationRSA::handleLinkBreakSendRERR(const L3Address& unreachableAddr)
{
...
const Ptr<Rerr> FabricationRSA::createRERR(const std::vector<UnreachableNode>& unreachableNodes)
{
...
void FabricationRSA::handleRERR(const Ptr<const Rerr>& rerr, const L3Address& sourceAddr)
{
...
void FabricationRSA::handleStartOperation(LifecycleOperation *operation)
{
    rebootTime = simTime();
...
void FabricationRSA::handleStopOperation(LifecycleOperation *operation)
{
    socket.close();
    clearState();
}

void FabricationRSA::handleCrashOperation(LifecycleOperation *operation)
{
    socket.destroy();
    clearState();
}

void FabricationRSA::clearState()
{
...
void FabricationRSA::handleWaitForRREP(WaitForRrep *rrepTimer)
{
...
void FabricationRSA::forwardRREP(const Ptr<RrepSec>& rrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
void FabricationRSA::forwardRREQ(const Ptr<RreqSec>& rreq, unsigned int timeToLive)
{
...
void FabricationRSA::completeRouteDiscovery(const L3Address& target)
{
...
void FabricationRSA::sendGRREP(const Ptr<RrepSec>& grrep, const L3Address& destAddr, unsigned int timeToLive)
{
...
const Ptr<RrepSec> FabricationRSA::createHelloMessage()
{
...
void FabricationRSA::sendHelloMessagesIfNeeded()
{
...
void FabricationRSA::handleHelloMessage(const Ptr<RrepSec>& helloMessage)
{
...
void FabricationRSA::expungeRoutes()
{
    for (int i = 0; i < routingTable->getNumRoutes(); i++) {
        IRoute *route = routingTable->getRoute(i);
        if (route->getSource() == this) {
...
void FabricationRSA::scheduleExpungeRoutes()
{
...
INetfilter::IHook::Result FabricationRSA::datagramForwardHook(Packet *datagram)
{
...
void FabricationRSA::sendRERRWhenNoRouteToForward(const L3Address& unreachableAddr)
{
    if (rerrCount >= rerrRatelimit) {
        EV_WARN << "A node should not generate more than RERR_RATELIMIT RERR messages per second. Canceling sending RERR" << endl;
        return;
...
void FabricationRSA::cancelRouteDiscovery(const L3Address& destAddr)
{
...
bool FabricationRSA::updateValidRouteLifeTime(const L3Address& destAddr, simtime_t lifetime)
{
    IRoute *route = routingTable->findBestMatchingRoute(destAddr);
    if (route && route->getSource() == this) {
        AodvRouteData *routeData = check_and_cast<AodvRouteData *>(route->getProtocolData());
...
const Ptr<RrepAck> FabricationRSA::createRREPACK()
{
...
void FabricationRSA::sendRREPACK(const Ptr<RrepAck>& rrepACK, const L3Address& destAddr)
{
    EV_INFO << "Sending Route Reply ACK to " << destAddr << endl;
    sendAODVPacket(rrepACK, destAddr, 100, 0);
}

void FabricationRSA::handleRREPACK(const Ptr<const RrepAck>& rrepACK, const L3Address& neighborAddr)
{
...
void FabricationRSA::handleRREPACKTimer()
{
    // when a node detects that its transmission of a RREP message has failed,
    // it remembers the next-hop of the failed RREP in a "blacklist" set.
...
void FabricationRSA::handleBlackListTimer()
{
